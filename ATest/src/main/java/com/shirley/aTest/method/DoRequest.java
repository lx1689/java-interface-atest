...
public class DoRequest {
	...
	public AssertResult toAssert(ResponseContent responseContent) {
		...
		System.out.println(responseContent.getResponseCode());
		
		JSONObject jo = JSONObject.parseObject(responseContent.getContent().toString());
		AssertMehtod assertMehtod = new AssertMehtod();
		for (Asserts asserts : assertList) {
			String status = "fail";
			if (jo.containsKey(asserts.getKey())) {
				status = assertMehtod.doAssert(getValue.getValue(jo.getString(asserts.getKey())), getValue.getValue(asserts.getValue()),
